...
def process_download(url, output, pages, delay, scale, quiet, is_batch=False, threads=1, non_interactive=False, job_id=None):
    ...
    with sync_playwright() as p:
        ...
        if not selected_pages_str:
            if is_batch or non_interactive:
        ...
        if job_id:
            result = {
                ...
                "title": doc_title,
                "url": url,
                "pages_requested": selected_pages_str,
                "pages_done": len(page_indices),
                "filename": output_file,
            }
            result_dir = os.path.dirname(output_file) or "."
...
@click.option('--url', 'url_opt', default=None, help='Scribd URL (action mode)')
...
def main(url, file, threads, output, pages, delay, scale, quiet, url_opt=None, non_interactive=False, job_id=None):
    ...
    if url_opt and not url:
        url = url_opt
    if non_interactive and not pages:
        pages = "all"
    ...
    if is_batch_mode and len(urls_to_process) > 1:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_url = {
                executor.submit(process_download, item_url, None, pages, delay, scale, quiet, True, final_threads, non_interactive, job_id): item_url
        ...
        for idx, item_url in enumerate(urls_to_process, start=1):
            ...
            if process_download(item_url, output if len(urls_to_process) == 1 else None, pages, delay, scale, quiet, is_batch=is_batch_mode, threads=final_threads, non_interactive=non_interactive, job_id=job_id):
                successful += 1
